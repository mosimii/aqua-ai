...
# === ✅ Cached RAG Retriever Builder ===
@st.cache_resource
def build_retriever():
    def fetch_html_paragraphs(url):
    ...
    # def fetch_local_pdf_text(path):
    #     with open(path, "rb") as f:
    #         text = extract_text(f)
    #     return text

    sources = [
        {"source": "Aquatrust – Testing Explainer", "url": "https://www.aquatrust.co.uk/legionella-testing-explainer-guide/"},
        {"source": "Aquatrust – Temperature Guide", "url": "https://www.aquatrust.co.uk/your-guide-to-water-temperature-and-legionella/"},
        {"source": "Aquatrust – Monitoring Services", "url": "https://www.aquatrust.co.uk/services-by-type/legionella-control/legionella-monitoring/"},
        {"source": "Aquatrust – Risk Assessment", "url": "https://www.aquatrust.co.uk/services-by-type/legionella-control/legionella-risk-assessment/"},
        {"source": "HSE UK – Legionella", "url": "https://www.hse.gov.uk/healthservices/legionella.htm"},
        {"source": "GOV.UK Safe Water Guidance", "url": "https://www.gov.uk/government/publications/guidance-for-organisations-on-supplying-safe-water-supplies/a-safe-water-supply-information-for-all-organisations"}
        # {"source": "WHO Water Safety PDF", "local_path": "who_legionella.pdf", "is_pdf": True}
    ]

    all_docs = []
    for s in sources:
        text = fetch_html_paragraphs(s["url"])
        doc = Document(page_content=text, metadata={"source": s["source"], "url": s["url"]})
        all_docs.append(doc)
    ...
    vectordb = Chroma.from_documents(split_docs, embedding=embedding_model)

    return vectordb

def answer_with_rag(query, retriever):
...
# === ✅ Streamlit UI Block ===
st.header("⁉️ Risk Guidance Assistant (Flan T5 Small)")

query_input = st.text_input(
    "Ask a question based on sensor readings and get answers using official water risk guidance:",
    value="Is Station STN001 at risk based on temperature 13.8 °C and chlorine 0.41 mg/L?"
)

if query_input:
    retriever = build_retriever()
    answer, docs = answer_with_rag(query_input, retriever)
